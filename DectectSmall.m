function locationSmall = DectectSmall(edge_img, img,RLearners, RWeights)
[h,w] = size(edge_img);
th_h = max(0.05*h,25);
se = strel('line',3,0);
...
for i = 1:num_chain
    ...
    occupy_value = sum(chain_img(:)~=0)/height_rotate/width_rotate;
    flag_occupy = occupy_value>1/3;
    ...
    if ~flag_occupy||~flag_num||flag_position
        continue
    end
