function Evaluation_recognition()
clear
clc

dir_gt = dir('E:\2012 文字检测\测试集\ICDAR 2011\test-textloc-gt\*.txt');
dir_es = dir('E:\2013毕设文字检测\试验结果\一起训练\location14\*.txt');
...
for index = 1:num_img
    ...
    es_name = ['E:\2013毕设文字检测\试验结果\一起训练\location14\' dir_es(index).name];
    % 读groundtruth坐标
    fid = fopen(gt_name);
    ...
    for i = 1:num_es
        ...
        str = '';
        if num_pre_one == 1
            str = txtGT{flag_pre_one};
            %             value_each_p = value_each_p + 0.8*double(sum(pm_vec(flag_pre_one))>tp);
        elseif num_pre_one > 1
            indexR = find(flag_pre_one);
            [~,indexS] = sort(lc_gt(indexR,1));
            for j = 1:num_pre_one
                str = [str txtGT{indexR(indexS(j))}];
            end
        end
        value_each_p = value_each_p + strcmp(upper(txtES(i)),upper(str));
        %             [~,index_des] = max(pm_vec);
        %             value_each_p = value_each_p + double(pm_vec(index_des)>tp&&sum(rm(index_des,:))>tr);
        %         end
    end
    p_each(index) = value_each_p/(num_es+eps);
    ...
    for i = 1:num_gt
        ...
        str = '';
        if num_rec_one == 1
            str = txtES{flag_rec_one};
        elseif num_rec_one > 1
            indexR = find(flag_rec_one);
            [~,indexS] = sort(lc_es(indexR,1));
            for j = 1:num_rec_one
                str = [str txtES{indexR(indexS(j))}];
            end
        end
        value_each_r = value_each_r + strcmp(upper(txtGT(i)),upper(str));
    end
